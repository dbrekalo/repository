	...
	var repo = {
		...
		load: function(params){
			...
			var sufix = repo.load.sufix,
				param_names = ['load', 'nope', 'yep'],
				prepareUrl = function(partial) {
					return (partial.indexOf('http') === 0 ? '' : baseUrl) + partial + sufix;
				};
			...
			$.each(param_names, function(i, item){
				...
				if ($.isArray(params[item])) {
					...
					$.each(params[item], function(i){ params[item][i] = prepareUrl(params[item][i]); });

				} else {

					params[item] = prepareUrl(params[item]);
